...
export default function PaymentHistoryPage() {
  ...
  return (
    <main className="flex flex-1 flex-col gap-4 px-4 pb-6 pt-5">
      ...
      {payments?.length === 0 && (
        <div className="flex flex-1 flex-col items-center justify-center gap-3 rounded-3xl border border-slate-800 bg-slate-900 p-10 text-center">
          <ShoppingCart className="h-10 w-10 text-slate-500" strokeWidth={1.5} />
          <p className="text-sm font-bold text-white">결제내역이 없습니다</p>
          <p className="text-xs text-slate-500">크레딧을 충전하고 캐릭터와 대화를 나눠보세요!</p>
          <Link
            to="/shop"
        ...
        <ul className="flex flex-col gap-2">
          {payments.map((payment) => (
            ...
            >
              ...
              <span className="rounded-full bg-slate-800 px-3 py-1 text-[11px] font-semibold text-slate-400">
