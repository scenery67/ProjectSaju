  ...
  return (
    <main className="flex flex-1 flex-col gap-4 px-4 pb-6 pt-5">
      <div className="flex flex-col gap-1">
        ...
        <p className="text-xs text-slate-400">공지와 크레딧 소식을 여기서 확인하세요.</p>
      </div>
      ...
      <div className="flex gap-2">
        <button
          ...
          className={`rounded-full px-4 py-1.5 text-xs font-bold ${
            tab === 'unread' ? 'bg-violet-600 text-white' : 'border border-slate-800 text-slate-400'
          }`}
        >
          안 읽음
        </button>
        <button
          ...
          onClick={() => setTab('read')}
          className={`rounded-full px-4 py-1.5 text-xs font-bold ${
            tab === 'read' ? 'bg-violet-600 text-white' : 'border border-slate-800 text-slate-400'
          }`}
        >
      ...
      {notifications === null && <p className="text-xs text-slate-400">불러오는 중...</p>}
      ...
      {notifications !== null && visible.length === 0 && (
        <div className="flex flex-1 flex-col items-center justify-center gap-2 rounded-3xl bg-white/[0.03] p-10 text-center">
          ...
          <p className="text-xs text-slate-500">새 알림이 도착하면 이곳에 표시됩니다.</p>
        </div>
      )}
