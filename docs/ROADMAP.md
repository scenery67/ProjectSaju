...
**📅 최종 업데이트**: 2026-09-02
**📊 진행 상황**: Phase 5 진행 중 (7/13 Tasks 완료, 과금은 백엔드만 우선 완료)
...
- **Task 012: 로그인 연동 마이페이지** - 부분 완료 (계정/크레딧/상점/결제내역 붙임, 서버 저장 사주 기록 조회는 아직)
  - ✅ 로그아웃, 로그인 계정 표시(닉네임/provider) — 기존 기능
  - ✅ 크레딧 잔액, 상점(패키지 목록), 결제내역 UI 추가 — Task 008에서 만든 `BillingController` API를 처음으로 소비
  - ✅ 상용화 전 임시 관리자 우회 로그인(`/api/auth/dev-admin-login`, `ADMIN_BYPASS_ENABLED` 플래그, 기본값 false) — 실제 OAuth 앱 등록 없이 팀이 관리자 권한으로 써볼 수 있게 함
  - 부수 발견: permitAll 경로에서 발생한 에러가 컨테이너 `/error` 포워딩을 타면서 Spring Security가 401로 덮어쓰던 버그 수정(`/error`를 permitAll에 추가) — `GET /api/saju/breakup`이 405 대신 401 반환하던 원인과 동일
  - 남음: "내 사주" localStorage 히스토리와 서버 저장 기록의 관계 정리(마이그레이션 또는 병행)
...
- **Task 008: LLM 사주 상담 + 질문 횟수 기반 과금** - 백엔드 완료(2026-09-01), 프론트 일부 반영(2026-09-02)
  ...
  - ✅ 마이페이지에 크레딧 잔액/상점/결제내역 UI 연결 — PG 연동 전이라 결제는 PENDING만 생성됨
  - 지금까지 계산한 사주 데이터(4주·오행·십성·대운·성격 프로필 등) 전부를 LLM 입력으로 구성해 상담 응답을 생성하는 기능 자체는 아직 미착수
  - PG사 실제 연동, 관리자 전용 화면(현재는 API만 존재), 상담(채팅) 로그 테이블은 남음 — 상담 로그는 LLM 상담 기능 설계가 먼저 필요
