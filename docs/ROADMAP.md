...
**📅 최종 업데이트**: 2026-09-02
**📊 진행 상황**: Phase 5 완료, Phase 6 진행 중 (8/13 Tasks 완료)
...
- **Task 008: LLM 사주 상담 + 질문 횟수 기반 과금** - 백엔드 완료(2026-09-01), 프론트 일부 반영(2026-09-02)
  ...
  - PG사 실제 연동, 상담(채팅) 로그 테이블은 남음 — 상담 로그는 LLM 상담 기능 설계가 먼저 필요. 환불 버튼은 완료 상태 결제가 아직 없어(PG 미연동) 브라우저 클릭까지는 검증 못 함(백엔드 로직은 `CreditServiceTest`로 커버)
