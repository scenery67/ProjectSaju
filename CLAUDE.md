...
- 저장은 서비스에 실제 필요한 필드만. 이름·연락처·이메일은 **수집하지 않는다**(인증 도입 전까지).
- 보관 기간 정책을 정하기 전까지 `reading_record`에 식별자(쿠키 ID, 세션 ID 등)를 추가하지 않는다.
...
- **커밋·푸시 금지.** `.claude/settings.json`에서 `git commit`/`git push`가 차단되어 있다. 변경은 작업 트리에 남기고 사람이 커밋한다.
